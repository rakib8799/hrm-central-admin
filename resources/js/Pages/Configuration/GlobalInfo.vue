        ...
        <div class="card mb-5 mb-xl-10" id="kt_profile_details_view">
            ...
            <VForm @submit="submit()" class="form">
                ...
                <div class="card-body border-top p-9">
                    <!-- Domain Admin Portal -->
                    <div class="row mb-5">
                        <label class="required fs-5 fw-semibold">{{ $t('configuration.label.domainAdminPortal') }}</label>
                    ...
                    <!-- Company Http Protocol -->
                    <div class="row mb-5">
                        <label class="required fs-5 fw-semibold">{{ $t('configuration.label.httpProtocol') }}</label>
                        <Field type="text" :placeholder="$t('configuration.placeholder.httpProtocol')" name="company_http_protocol" class="form-control form-control-lg form-control-solid" v-model="formData.company_http_protocol"/>
                        <ErrorMessage :errorMessage="formData.errors.company_http_protocol"/>
                    </div>
                    <!-- Support Email -->
                    <div class="row mb-5">
                        <label class="required fs-5 fw-semibold">{{ $t('configuration.label.supportEmail') }}</label>
                    ...
                    <!-- Support Telephone -->
                    <div class="row mb-5">
                        <label class="required fs-5 fw-semibold">{{ $t('configuration.label.supportTelephone') }}</label>
                ...
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    <SubmitButton :id="props.configuration?.id" />
                </div>
            </VForm>
        </div>
    </AuthenticatedLayout>
...
const formData = useForm({
    ...
    company_http_protocol: props.configuration?.company_http_protocol || '',
